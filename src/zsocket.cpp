...
void ZSocket::__construct(Php::Parameters &param) {
	const char *endpoint = (param.size() > 1) ? param[1].stringValue().c_str() : NULL;
	void *socket = ZSocket::new_socket(param[0].stringValue().c_str(), endpoint);
...
Php::Value ZSocket::new_socket_php(std::string type, Php::Parameters &param) {
	const char *endpoint = (param.size() > 0) ? param[0].stringValue().c_str() : NULL;
	zsock_t *socket = ZSocket::new_socket(type.c_str(), endpoint);
	// int inttype = zsock_type(socket);
	// zsys_info("Building Socket type: %s [%d]", type.c_str(), inttype);
...
zsock_t *ZSocket::new_socket(const char* name, const char* endpoint) {
	...
	zsock_t *socket = NULL;
	std::string _name = toLower(name);

	if(_name == "pub")
		socket = zsock_new_pub(endpoint);
	else
	if(_name == "sub")
		socket = zsock_new_sub(endpoint, NULL);
	else
	if(_name == "req")
		socket = zsock_new_req(endpoint);
	else
	if(_name == "rep")
		socket = zsock_new_rep(endpoint);
	else
	if(_name == "dealer")
		socket = zsock_new_dealer(endpoint);
	else
	if(_name == "router")
		socket = zsock_new_router(endpoint);
	else
	if(_name == "push")
		socket = zsock_new_push(endpoint);
	else
	if(_name == "pull")
		socket = zsock_new_pull(endpoint);
	else
	if(_name == "xpub")
		socket = zsock_new_xpub(endpoint);
	else
	if(_name == "xsub")
		socket = zsock_new_xsub(endpoint);
	else
	if(_name == "xreq")
		socket = zsock_new_dealer(endpoint);
	else
	...
	if(_name == "pair")
		socket = zsock_new_pair(endpoint);
	else
	if(_name == "stream")
		socket = zsock_new_stream(endpoint);
	else
		throw Php::Exception("error: can't create socket type: " + _name);

	return (socket == NULL) ? nullptr : socket;
}
