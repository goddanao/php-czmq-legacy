#include "../../include/majordomo/mdp_worker.h"
...
void MajordomoWorker::__construct(Php::Parameters &param) {
    if(param.size() > 0)
        verbose = (int) param[0];
}

void MajordomoWorker::send_to_broker (char *command, char *option, zmsg_t *msg){
...
void MajordomoWorker::connect_to_broker () {
	if (worker)
		zsock_destroy (&worker);
...
void MajordomoWorker::run(Php::Parameters &param) {
...
zmsg_t * MajordomoWorker::recv (zframe_t **reply_to_p) {
...
void MajordomoWorker::send (zmsg_t **report_p, zframe_t *reply_to) {
    assert (report_p);
