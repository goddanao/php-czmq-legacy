...
private:
    ...
    static Php::Value decode_phpvalue(msgpack::object *unpacked, Php::Value &c) {
       ...
       std::function<Php::Value(int type, Php::Value v, Php::Value a)> cb = [&c, custom_decoder](int type, Php::Value v, Php::Value a){
        if(!custom_decoder)
            return v;
       ...
       switch(unpacked->type) {
           case MSGPACK_OBJECT_NIL: return cb(unpacked->type, nullptr, {});
           case MSGPACK_OBJECT_BOOLEAN: return cb(unpacked->type,unpacked->via.boolean, {});
           case MSGPACK_OBJECT_POSITIVE_INTEGER: return cb(unpacked->type,(int64_t) unpacked->via.u64, {});
           case MSGPACK_OBJECT_NEGATIVE_INTEGER: return cb(unpacked->type,(int64_t) unpacked->via.i64, {});
           case MSGPACK_OBJECT_FLOAT: return cb(unpacked->type,unpacked->via.f64, {});
           case MSGPACK_OBJECT_STR: return cb(unpacked->type,Php::Value(unpacked->via.str.ptr, unpacked->via.str.size), {});
           case MSGPACK_OBJECT_BIN: return cb(unpacked->type,Php::Value(unpacked->via.bin.ptr, unpacked->via.bin.size), {});
           case MSGPACK_OBJECT_ARRAY:
               for(int i = 0; i < unpacked->via.array.size; i++)
                   result[i] = decode_phpvalue(unpacked->via.array.ptr+i, c);
               return cb(unpacked->type, result, {});
           break;
           case MSGPACK_OBJECT_MAP:
               ...
               if(classname != "")
                return cb(unpacked->type, result, {classname});
               else
                return cb(unpacked->type, result, { });
           break;
           case MSGPACK_OBJECT_EXT:
               result = Php::Value(unpacked->via.ext.ptr, unpacked->via.ext.size);
               return cb(unpacked->type, result, { unpacked->via.ext.type() });
           break;

           default:
            return cb(unpacked->type, nullptr, {});
           break;
       }
   }
...
//
//        std::function<Php::Value(const int type, Php::Value v)> custom_decoder = [got_custom_decoder, &c](const int type, Php::Value v) {
//           if(got_custom_decoder) {
//               Php::Value result = c(type, v);
//               zsys_info("Got custom decoder for type")
//               return result.isNull() ? v : result;
//           }
//           return v;
//        };
//
//        std::function<Php::Value(const int type, Php::Value k, Php::Value v)> custom_decoder_2 = [got_custom_decoder, &c](const int type, Php::Value k, Php::Value v) {
//           if(got_custom_decoder) {
//               Php::Value result = c(type, k, v);
//               return result.isNull() ? v : result;
//           }
//           return v;
//        };
...
//               }
//
//               return custom_decoder_2(unpacked->type, result, classname);
//
//            case MSGPACK_OBJECT_EXT: return custom_decoder_2(unpacked->type, Php::Value(unpacked->via.ext.ptr, unpacked->via.ext.size), unpacked->via.ext.type());
//            default: return custom_decoder(unpacked->type, nullptr);
//        }
//    }
//
//    static Php::Value decode_from_buffer(const void *data, size_t data_len, Php::Value &c) {
